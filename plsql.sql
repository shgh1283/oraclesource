...
-- PL/SQL : DATABASE 관련 특정 작업을 수행하는 명령어들을 / 선언문을 모아둔 블럭
-- sql + 프로그래밍
...
--	DBMS_OUTPUT.PUT_LINE('V_EMPNO : ' || V_EMPNO );
	DBMS_OUTPUT.PUT_LINE('V_NAME : ' || V_ENAME );
END;
/
...
DECLARE
	V_TAX CONSTANT NUMBER(2) DEFAULT 10;
BEGIN
	DBMS_OUTPUT.PUT_LINE('V_TAX : ' || V_TAX );
END;
/
...
DECLARE
--	V_TAX CONSTANT NUMBER(2) DEFAULT 10;
	V_TAX CONSTANT NUMBER(2) NOT NULL := 20;
BEGIN
	DBMS_OUTPUT.PUT_LINE('V_TAX : ' || V_TAX );
END;
/
...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('V_NUMBER : ' || V_NUMBER || ' (은)는 홀수');
	ELSE
	DBMS_OUTPUT.PUT_LINE('V_NUMBER : ' || V_NUMBER || ' (은)는 짝수');
END IF;
END;
/
...
BEGIN
	FOR i IN 0..10 LOOP
	IF MOD(i,2) = 1 THEN 
	DBMS_OUTPUT.PUT_LINE('i : ' || i || ' (은)는 홀수');
END IF;
END LOOP;
END;
/

-- 커서(cursor) : SELCT 문 또는 DML 과 같은 SQL 구문 실행 했을 때 해당 SQL을 처리하는
-- 정보를 저장한 메모리 공간
...
BEGIN	
	...
	DBMS_OUTPUT.PUT_LINE('DEPTNO : ' || C1_ROW.DEPTNO || ' DNAME : ' || C1_ROW.DNAME || ' LOC : ' || C1_ROW.LOC);	
	END LOOP;
END;
/
